...
class ArmEnv(object):
    ...
    def step(self, action):
        ...
        finger = np.array([np.cos(a2r + a3r), np.sin(a2r + a3r)]) * a3l + a2xy_
        # normalize features
...
class Viewer(pyglet.window.Window):
    ...
    def __init__(self, arm_info, goal):
        ...
        super(Viewer, self).__init__(width=400, height=400, resizable=False, caption='触手', vsync=False)
        ...
        self.arm2 = self.batch.add(
            ...
            ('v2f', [100, 150,              # location
                     100, 160,
                     200, 160,
                     200, 150]), ('c3B', (86, 86, 249) * 4,))
        self.arm3 = self.batch.add(
            4, pyglet.gl.GL_QUADS, None,
            ('v2f', [50, 50,              # location
                     50, 150,
                     60, 150,
                     60, 50]), ('c3B', (249, 206, 6) * 4,))
    ...
    def _update_arm(self):
        ...
        a1xy_ = np.array([np.cos(a1r), np.sin(a1r)]) * a1l + a1xy   # a1 end and a2 start (x1, y1)
        a2xy_ = np.array([np.cos(a1r+a2r), np.sin(a1r+a2r)]) * a2l + a1xy_  # a2 end (x2, y2)
        a3xy_ = np.array([np.cos(a2r + a3r), np.sin(a2r + a3r)]) * a3l + a2xy_

        a1tr, a2tr, a3tr = np.pi / 2 - self.arm_info['r'][0], np.pi / 2 - self.arm_info['r'][1], np.pi / 2 - self.arm_info['r'].sum()
        ...
        xy21_ = a2xy_ + np.array([-np.cos(a2tr), np.sin(a2tr)]) * self.bar_thc
        xy22_ = a2xy_ + np.array([np.cos(a2tr), -np.sin(a2tr)]) * self.bar_thc
        xy32 = a3xy_ + np.array([np.cos(a3tr), -np.sin(a3tr)]) * self.bar_thc
        xy33 = a3xy_ + np.array([-np.cos(a3tr), np.sin(a3tr)]) * self.bar_thc

        self.arm1.vertices = np.concatenate((xy01, xy02, xy11, xy12))
        self.arm2.vertices = np.concatenate((xy11_, xy12_, xy21, xy22))
        self.arm3.vertices = np.concatenate((xy21_, xy22_, xy32, xy33))
